...
def read_fin_history():
    ...
    terminal_listbox.insert(END, "입력받은 주식코드({})의 데이터를 가져옵니다.".format(STOCK_CODE))
...
def start_Learning():  # 머신러닝 시작
        ...
        def print_history(self, step, dic_history={}):
            ...
            if self.in_count < self.epoch_count:
                ...
                terminal_listbox.insert(
                    END,
                    "Epoch: {:4}/{} ... Iteration: {:4}/{}\n{}".format(
                        self.epoch_count, Epoch_num, step, self.Iteration, metrics_log
                    ),
                )
                self.in_count += 1
    ...
    while (count < Epoch_num) and (while_TF_early_stop):
        ...
        step = cal_step.step
        terminal_listbox.insert(END, out_batch.print_history(step, history.history))
...
btn_read_data = Button(
    file_frame, padx=5, pady=5, width=12, text="데이터 읽어오기", command=read_fin_history
).pack(side="right")
...
terminal_listbox.insert(tk.END, "# GOOGLE(A_Class) : GOOGL / GOOGLE(C_Class) : GOOG")
terminal_text = ">> "
...
btn_close = Button(
    frame_run, padx=5, pady=5, text="닫기", width=12, command=root.quit
).pack(side="right", padx=5, pady=5)
btn_start = Button(
    frame_run, padx=5, pady=5, text="표 출력", width=12, command=show_graph
).pack(side="right", padx=5, pady=5)
btn_start = Button(frame_run, padx=5, pady=5, text="시작", width=12, command=start).pack(
